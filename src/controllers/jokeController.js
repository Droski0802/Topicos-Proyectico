...
const getJokesByScore = async (req, res) => {
  ...
  try {
    ...
    if (!jokes.length) {
      return res.status(404).json({ message: 'No se encontraron chistes con ese puntaje' });
    }

    res.status(200).json(jokes);
  } catch (error) {
    console.error('Error al obtener los chistes:', error);
    return res.status(500).json({ error: 'Error al obtener los chistes', details: error.message });
  }
};
